...
# Define colors
white = (255, 255, 255)
dark_yellow = (204, 204, 0)  # Darker yellow for score text
black = (0, 0, 0)
red = (213, 50, 80)
green = (0, 255, 0)
blue = (50, 153, 213)
...
def our_score(score, lives, level, total_score):
    ...
    text_surface = score_font.render(text, True, dark_yellow)  # Dark yellow text
...
def our_snake(snake_block, snake_list, current_direction):
    ...
    # Rotation logic for the head based on current direction
    if current_direction == 'RIGHT':
     ...
     # If the snake has exactly 2 segments (head and tail)
    if len(snake_list) == 2:
        tail = snake_list[0]  # First element is the tail
        # Determine the direction of the tail based on the second segment (the head)
...
def display_final_score(total_score):
    final_score = score_font.render("Total Score: " + str(total_score), True, dark_yellow)
